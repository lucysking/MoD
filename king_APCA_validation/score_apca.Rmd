...
apca_raw_file <- "~/Box/Mooddata_Coordinating/MOD/Data/REDCap/APCA/apca_raw_data_20210511_cleaned.csv"
apca_start_time <- "~/Box/Mooddata_Coordinating/MOD/Data/REDCap/APCA/raw_data/apca_start_time_20210511.csv"
apca_end_time <- "~/Box/Mooddata_Coordinating/MOD/Data/REDCap/APCA/raw_data/apca_end_time_20210511.csv"
...
- Whether child heard or saw 38 of these events (excluded foster case/adoption [childhood only], bullying [childhood only], ill_child_exp_dir [happened directly to target child], job [parent being fired/liad off -- very unlikely to be seen or heard]; money [cannot be seen or heard])
...
a <-
  ...
  left_join(
    read_csv(
      ...
      filter(!is.na(sdq_timestamp)) %>% 
      rename(apca_start_time = sdq_timestamp),
    by = "id"
  ) %>% 
  left_join(
    read_csv(
      ...
      dplyr::select(
        id,
        apca_end_time = apca_timestamp
      ),
    by = "id"
  ) %>% 
  mutate(
    apca_start_time = parse_date_time(apca_start_time, "mdy HM"),
    apca_end_time = parse_date_time(apca_end_time, "mdy HM"),
    apca_duration  = (apca_start_time %--% apca_end_time) / minutes(1),
    # 3 participants completed the APCA on a different date than other measures and therefore are missing timing info;
    # 1 participant clearly left and came back to the survey and is an outlier
    apca_duration = case_when(
      id == 10546 ~ NA_real_,
      id == 10096 ~ NA_real_,
      id == 10732 ~ NA_real_,
      id == 10074 ~ NA_real_,
...
quantile(a$apca_duration, na.rm = TRUE)
```

```{r}
SpearmanRho(a$parent_num_types, a$apca_duration, conf.level = .95, use = "complete.obs")
SpearmanRho(a$child_num_dir_types, a$apca_duration, conf.level = .95, use = "complete.obs")
```
...
write_csv(
  a, 
  "~/Box/Mooddata_Coordinating/MOD/Data/REDCap/APCA/apca_scored_20210514.csv"
)
